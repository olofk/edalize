...
def test_oxide_multiple_pdc(make_edalize_test):
    files = [{'name': 'pdc_file.pdc', 'file_type': 'PDC'},
    ...
    with pytest.raises(RuntimeError) as e:
        tf.backend.configure()
    assert "Nextpnr only supports one PDC file. Found pdc_file.pdc and pdc_file2.pdc" in str(e.value)
