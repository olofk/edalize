    ...
    def configure_main(self):
        ...
        if self.synth_tool == "yosys":
            ...
            self.render_template(
                "vivado-project-yosys.tcl.j2",
                self.name + ".tcl",
                {"name": self.name, "tool_options": self.tool_options},
            )

            self.render_template(
                "vivado-run-yosys.tcl.j2",
                self.name + "_run.tcl",
                {
            ...
            self.render_template(
                "vivado-project.tcl.j2", self.name + ".tcl", template_vars
            )
        ...
        vivado_command = (
            "source {} && vivado".format(vivado_settings)
            if vivado_settings
            else "vivado"
        )
