...
class Filelist(Edatool):
    ...
    def configure_main(self):
        ...
        with outfile.open('w') as f:
            ...
            (src_files, incdirs) = self._convert_paths(*self._get_fileset_files(force_slash=True), mode)
    ...
    def _resolve_env_var(self, outpath):
        ...
        if match := regex.match(outpath):
            ...
            try:
                ...
                logger.error('could not resolve environment variable in outpath="{}"'.format(match.group(1)))
                raise RuntimeError()
        else:
            logger.error('did not recognise environment variable in outpath="{}"'.format(outpath))
            raise RuntimeError()
    ...
    def _convert_paths(self, src_files, incdirs, mode):
        ...
        for src_file in src_files:
            src_file.name = self._resolve_path(Path(self.work_root) / src_file.name, mode)
    ...
    def _resolve_path(self, path, mode):
        ...
        if mode == 'absolute':
            ...
            return os.path.relpath(path.resolve(), start=self.tool_options['outpath'].resolve())
        else:
