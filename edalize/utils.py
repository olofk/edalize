class EdaCommands(object):
    class Command(object):
        def __init__(
            self, commands, targets, depends, order_only_deps=None, variables=None
        ):
            self.commands = commands
            self.targets = targets
            self.depends = depends
            self.order_only_deps = list(order_only_deps) if order_only_deps else []
    ...
    def add(self, command, targets, depends, order_only_deps=None, variables=None):
        if command and type(command[0]) == list:
            commands = command
        else:
            commands = [command]
        self.commands.append(
            self.Command(commands, targets, depends, order_only_deps, variables)
        )
    ...
    def add_env_var(self, key, value):
        self.variables.append(f"{self.find_env_var_command()} {key}={value}")
